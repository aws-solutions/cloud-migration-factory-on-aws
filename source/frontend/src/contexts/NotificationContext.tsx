...
export type NotificationContextType = {
  ...
  setNotifications: (value: (((prevState: FlashbarProps.MessageDefinition[]) => FlashbarProps.MessageDefinition[]) | FlashbarProps.MessageDefinition[])) => void;
};
  ...
  const addNotification = (notificationAddRequest: CmfAddNotification) => {
    ...
    if (notificationAddRequest.actionButtonLink && notificationAddRequest.actionButtonTitle) {
      notification.action = (<Button
        onClick={event => navigateClick(event, notificationAddRequest.actionButtonLink)}>{notificationAddRequest.actionButtonTitle}</Button>)
    }
