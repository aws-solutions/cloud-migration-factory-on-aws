...
function getColumnDefinitionsForGroups(attr, lattr) {
    return ({
            id: attr.name,
            header: attr.description,
            cell: item => getNestedValuePath(item, lattr.import_raw_header) ? getNestedValuePath(item, lattr.import_raw_header).map((group, index) => {
...
function getColumnDefinitionsDefault(attr, lattr) {
    return ({
            id: attr.name,
            header: <ExpandableSection header={attr.description}><SpaceBetween
                size={'xxs'}>{'File header: ' + lattr.import_raw_header}{'Schema: ' + lattr.schema_name}</SpaceBetween></ExpandableSection>,
            cell: item => getNestedValuePath(item, lattr.import_raw_header),
            minWidth: 180,
            sortingField: attr.name
        }
