...
import {deepEqual} from "./resources/main"
...
const App = (props) => {
  ...
  async function openUserSession() {
    ...
    if ('API_SSMSocket' in window.env) {
      ...
      if (window.env.API_VPCE_ID === ''){
        ...
        websocket = new WebSocket( 'wss://' + window.env.API_SSMSocket + '.execute-api.' + window.env.API_REGION + '.amazonaws.com/prod');
  ...
  const handleHelpContentChange: (content, silent?: boolean) => Promise<void> = async (content, silent = true) => {
    ...
    if(isAuthenticating || schemaIsLoading){
      ...
      setIsReady(false);
      if (!schemaError.startsWith('No current user')) {
        setError('Schema could not be loaded. ' + schemaError);
