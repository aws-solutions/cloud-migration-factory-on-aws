...
const App = () => {
  ...
  const [lastCentralNotificationDateTime, setLastCentralNotificationDateTime] = useState(localStorageLastNotificationDateTime ? JSON.parse(localStorageLastNotificationDateTime) : null);
  ...
  async function openUserSession(idToken: string) {
    ...
    serverNotificationTimerID = setInterval(
      () => serverNotificationsTick(),
      60000
    );
    ...
    if ('API_SSMSocket' in environment) {
      ...
      if (environment.API_VPCE_ID === '') {
        ...
        console.info('Jobs web socket not supported when using VPCE specified, no push notifications will be received.');
      }
  ...
  const tick = async () => {
      ...
      if (minutesSinceActivity > autoLogoutMinutes) {
        ...
        console.log('User has been inactive for ' + minutesSinceActivity + 'mins, reaching timeout of ' + autoLogoutMinutes + ' mins, logging out.')
  ...
  function displayAuthenticatedMainUI() {
    ...
    return <AppLayout
      ...
      contentType="table"
      tools={<ToolHelp
        helpContent={toolsState.toolsHelpContent}
      />}
