  ...
  return render(
    <MemoryRouter initialEntries={["/forgot/pwd"]}>
      <SessionContext.Provider value={NO_SESSION}>
        <div id="modal-root" />
        <Routes>
          <Route path={"/forgot/pwd"} element={<ForgotPassword></ForgotPassword>}>
            {" "}
          </Route>
          <Route path={"/login"} element={<>This is the login page</>}>
            {" "}
          </Route>
        </Routes>
...
test("it sends a reset code", async () => {
  ...
  expect(
    await screen.findByText("Check registered/verified email or phone number for password reset code.")
  ).toBeInTheDocument();
});
