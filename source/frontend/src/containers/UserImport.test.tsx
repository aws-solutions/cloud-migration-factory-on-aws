...
test("it should allow download the intake forms from the import page", async () => {
    ...
    expect(XLSX.writeFile).toHaveBeenCalledWith(expect.objectContaining({"SheetNames": ["mf_intake"]}), "cmf-intake-form-req.xlsx");
    ...
    expect(XLSX.writeFile).toHaveBeenLastCalledWith(expect.objectContaining({"SheetNames": ["mf_intake"]}), "cmf-intake-form-all.xlsx");
});
...
function setup_all_get_handlers() {
    server.use(
        ...
        rest.get("/user/server", (request, response, context) => {
            return response(
                context.status(200),
                context.json(generateTestServers(5))
            );
        }),
        rest.get("/user/app", (request, response, context) => {
            return response(
                context.status(200),
                context.json(generateTestApps(5))
            );
        }),
        rest.get("/user/database", (request, response, context) => {
            return response(
                context.status(200),
                context.json(generateTestDatabases(5))
            );
        }),
...
async function assert_upload_file_selected(fileName: string) {
    expect(screen.getByText((_: string, node: Element | null) => {
...
async function select_valid_file_upload_scenario1() {
    ...
    const validFileContents = "server_name,server_os_family,server_os_version,server_fqdn,r_type,app_name,aws_accountid,aws_region,wave_name\n" +
...
async function select_valid_xlsx_file_upload_scenario1() {
    ...
    await select_upload_file_click_next(validFileContents, "valid.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
}
...
async function select_valid_file_upload_scenario3() {
    ...
    const validFileContents = "server_name,server_os_family,server_os_version,server_fqdn,r_type,app_name,aws_accountid,aws_region,wave_name,wave_status\n" +
...
function assert_two_headers(headerText1: string, headerText2: string, count: number) {
    ...
    expect(screen.getAllByText((_: string, node: Element | null) => {
        // eslint-disable-next-line testing-library/no-node-access
        if (node && node.textContent === headerText1 && node.parentNode && node.parentNode.nextSibling && node.parentNode.nextSibling.textContent === headerText2) {
            return true;
        }
        return false;
        ...
        expect(addNotification).toHaveBeenCalledWith(expect.objectContaining({
            ...
            content:
                <ExpandableSection headerText="Error details">
                    <ExpandableSection
                        key="wave - Create failed"
                        headerText="wave - Create failed">
                        ["error_detail : Simulated Error 1,Simulated Error 2","error_detail : Unprocessed Item 1,Unprocessed Item 2"]
                    </ExpandableSection>
                    <ExpandableSection
                        key="application - Create failed"
                        headerText="application - Create failed">
                        ["field1 already exists.","field2 already exists."]
                    </ExpandableSection>
                    <ExpandableSection
                        key="server - Create failed"
                        headerText="server - Create failed">
...
test("Clicking Upload on the overview page, upload fails with api post 500 error - scenario1", async () => {
    ...
    expect(screen.getByRole("button", {name: "application Create - Internal API error - Contact support"})).toBeVisible();
...
test("Clicking Upload on the overview page, uploads - scenario3", async () => {
    ...
    await waitFor(() => {
        expect(addNotification).toHaveBeenCalledWith(expect.objectContaining({
            "content": <ProgressBar additionalInfo="Update wave records..."
                                    label="Importing file 'valid.csv' ..."
...
test("Clicking Upload on the overview page, uploads api errors - scenario3", async () => {
    ...
    expect(screen.getByRole("button", {name: "application - Unit testing App 1 - Unknown error occurred"})).toBeVisible();
...
async function select_valid_file_warnings_upload() {
    const validFileContents = "server_name,server_os_familyNO,server_os_version,server_fqdn,r_type,app_name,aws_accountid,aws_region,wave_name\n" +
        "unittest1,linux,redhat,unittest1.testdomain.local,Rehost,Unit testing App 1,123456789012,us-east-2,Unit testing Wave 1"
    await select_upload_file_click_next(validFileContents, "valid.csv", "text/csv");
}
...
async function select_invalid_file_upload() {
    const fileContents = "server_name,server_os_family,server_os_version,server_fqdn,r_type,app_name,aws_accountid,aws_region,wave_name,wave_status\n" +
