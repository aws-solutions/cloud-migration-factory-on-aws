...
const AutomationScripts = (props: AutomationScriptsParams) => {
  ...
  async function handleAction(e: ClickEvent) {
    ...
    if (action === "new_version") {
      ...
      setFocusItem(selectedItems[0]);
      await downloadScriptVersion({
        package_uuid: selectedItems[0].package_uuid,
        script_name: selectedItems[0].script_name,
        default: selectedItems[0].latest,
      });
    }
  }
  ...
  async function handleUpload(
    selectedFile: File,
    details: { script_name: any; package_uuid: any; __make_default: any; default: any }
  ) {
  ...
  function displayViewScreen() {
    return (
      <SpaceBetween direction="vertical" size="xs">
        ...
        />
        <ViewAutomationScript schema={schemaSSMAttribs} selectedItems={selectedItems} dataAll={dataAll} />
      </SpaceBetween>
    );
  }
      ...
      <SpaceBetween direction={"vertical"} size={"xxl"}>
        ...
        >
          <SpaceBetween size={"xxl"} direction={"vertical"}>
            <FormField key={"script_name"} label={"Script Name"}>
              <Input onChange={undefined} value={focusItem.script_name} disabled={true} />
            </FormField>
            <FormField key={"script_desc"} label={"Script Description"}>
              <Input onChange={undefined} value={focusItem.script_description} disabled={true} />
            </FormField>
        ...
        {newDefaultVersion !== focusItem.default ? (
          <Alert dismissAriaLabel="Close alert" type="warning">
            {newDefaultVersion !== focusItem.default
              ? "Saving will change default script to use version " +
                newDefaultVersion +
                " instead of version " +
                focusItem.default +
                " for all automation future jobs."
              : ""}
          </Alert>
        ) : undefined}
        <SpaceBetween direction={"horizontal"} size={"xxs"}>
          <Button variant={"primary"} onClick={handleResetScreen} disabled={false}>
            Cancel
          </Button>
          <Button onClick={handleChangeVersion} disabled={newDefaultVersion !== focusItem.default ? false : true}>
            Save
          </Button>
        </SpaceBetween>
      </SpaceBetween>
