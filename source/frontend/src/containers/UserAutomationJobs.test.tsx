      ...
      <MemoryRouter initialEntries={["/automation/jobs"]}>
        <NotificationContext.Provider value={mockNotificationContext}>
          <SessionContext.Provider value={TEST_SESSION_STATE}>
            <UserAutomationJobs {...defaultTestProps} />
            <div id="modal-root" />
...
test("it renders a paginated table with 50 jobs", async () => {
  server.use(
    rest.get("/ssm/jobs", (request, response, context) => {
      return response(context.status(200), context.json(generateTestAutomationJobs(50)));
    })
  );
...
test("clicking the refresh button refreshes the table", async () => {
  server.use(
    rest.get("/ssm/jobs", (request, response, context) => {
      return response.once(context.status(200), context.json(generateTestAutomationJobs(5)));
    }),
    rest.get("/ssm/jobs", (request, response, context) => {
      return response.once(context.status(200), context.json(generateTestAutomationJobs(8)));
    })
  );
...
test("it shows the details when a row is selected", async () => {
  server.use(
    rest.get("/ssm/jobs", (request, response, context) => {
      return response(context.status(200), context.json(generateTestAutomationJobs(5)));
    })
  );
...
test("clicking Submit Automation Job submits, api error with error text", async () => {
  server.use(
    rest.post("/ssm", (request, response, context) => {
      return response(context.status(500), context.text("Error posting"));
    })
  );
...
test("submit mgn server migration successfully", async () => {
  server.use(
    rest.post("/mgn", (request, response, context) => {
      return response(context.status(200), context.text("Submitted to mgn"));
    })
  );
...
test("submit mgn server migration api error", async () => {
  server.use(
    rest.post("/mgn", (request, response, context) => {
      return response(context.status(500), context.text("Errors submitting to mgn"));
    })
  );
