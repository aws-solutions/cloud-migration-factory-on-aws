...
test('it renders a paginated table with 50 servers', async () => {
  ...
  server.use(
    rest.get('/user/server', (request, response, context) => {
      return response(
        context.status(200),
        context.json(generateTestServers(50))
      );
    })
  );
  ...
  server.use(
    rest.get('/user/server', (request, response, context) => {
      return response.once(
        context.status(200),
        context.json(generateTestServers(1))
      );
    }),
...
test('submitting the add form saves a new server to API', async () => {
  ...
  server.use(
    rest.get('/user/server', (request, response, context) => {
      return response(
        context.status(200),
        context.json(generateTestServers(2))
      );
    }),
  ...
  await userEvent.type(screen.getByRole('textbox', {name: 'Server Name'}), 'my-test-server');
  await userEvent.type(screen.getByRole('textbox', {name: 'Server FQDN'}), 'foo');

  await userEvent.click(screen.getByLabelText('Application'));
  await userEvent.click(await screen.findByText('Unit testing App 1'));

  await userEvent.click(screen.getByRole('button', {name: /migration strategy info r_type select migration strategy/i}));
  ...
  await userEvent.type(screen.getByRole('textbox', {name: 'Server Environment'}), 'Production');
  await userEvent.type(screen.getByRole('textbox', {name: 'Server OS Version'}), 'Production');
...
test('submitting the edit form saves the server to API', async () => {
  ...
  const serverNameInput = screen.getByRole('textbox', {name: 'Server Name'});
