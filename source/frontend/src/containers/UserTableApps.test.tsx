...
test('it renders a paginated table with 50 applications', async () => {
  ...
  server.use(
    rest.get('/user/app', (request, response, context) => {
      return response(
        context.status(200),
        context.json(generateTestApps(50))
      );
    })
  );
  ...
  server.use(
    rest.get('/user/app', (request, response, context) => {
      return response.once(
        context.status(200),
        context.json(generateTestApps(1))
      );
    }),
...
test('submitting the add form saves a new application to API', async () => {
  ...
  await userEvent.type(screen.getByRole('textbox', {name: 'Application Name'}), 'my-test-application');
...
test('submitting the edit form saves the application to API', async () => {
  ...
  const applicationNameInput = screen.getByRole('textbox', {name: 'Application Name'});
...
test('delete multiple applications', async () => {
  ...
  // THEN
  const withinModal = within(await screen.findByRole('dialog'));
  expect(await withinModal.findByText('Are you sure you wish to delete the 2 selected applications?')).toBeInTheDocument();
