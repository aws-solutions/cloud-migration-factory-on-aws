...
import UserListWaves from "../components/UserListWaves"
import AppDialog from "../components/AppDialog";

export default class UserWaves extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  async getPermissions() {
    try {
      ...
      if (this.state.stages.length > 0 &&
        ...
        this.setState({
          showDialog: true,
          msg: "You are not authorizaed to manage wave list"
        });
      }
      }
    } catch (e) {
  ...
  updateWaveList = (action, wave_id) => {
    switch(action) {
      ...
      case 'delete':
        var newWaves = [];
        for ( var i in this.state.waves) {
          if (wave_id !== this.state.waves[i].wave_id) {
            newWaves.push(this.state.waves[i]);
          }
        }
        this.setState({
          waves: newWaves
