    ...
    try {
      ...
      if (window1.env.API_VPCE_ID !== '') {
        finalErrorText = e + ', when trying to access API Gateway VPC Endpoint URL https://' +
           window1.env.API_ADMIN + '-' + window1.env.API_VPCE_ID + '.execute-api.' + window1.env.API_REGION + '.amazonaws.com/prod. Please resolve the issue connecting from this device and retry.'
      } else {
        finalErrorText = e + ', when trying to access API Gateway URL ' +
          'https://' + window1.env.API_ADMIN + '.execute-api.' + window1.env.API_REGION + '.amazonaws.com/prod. Please resolve the issue connecting from this device and retry.'
      }
