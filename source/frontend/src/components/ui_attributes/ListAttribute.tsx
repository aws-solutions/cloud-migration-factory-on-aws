...
const ListAttribute = ({ attribute, value, isReadonly, errorText, handleUserInput, displayHelpInfoLink }) => {
  ...
  function handleUpdate(attribute, value, currentErrorText) {
    setLocalValue(value);
    handleUserInput(attribute, value, currentErrorText);
  }
      ...
      {attribute.listMultiSelect ? (
        ...
        <Grid gridDefinition={[{ colspan: 10 }, { colspan: 2 }]}>
          <Select
            ...
            onChange={(event) => handleUpdate(attribute, event.detail.selectedOption.value, currentErrorText)}
            statusType={getStatusType(attribute)}
            loadingText={"Loading values..."}
            errortext={currentVLErrorText ? currentVLErrorText : undefined}
            options={localOptions}
          ...
          <Button
            iconName="close"
            variant="normal"
            ariaLabel={attribute.name + "-clear"}
            disabled={isReadonly}
