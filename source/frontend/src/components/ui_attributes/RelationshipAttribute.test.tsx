...
const setup = () => {
  const comp =  render(
    <RelationshipAttribute
      schemas={schemas}
      attribute={attribute}
      value={{status: 'loaded', value: [], invalid: []}}
      record={[]}
      options={[]}
      isReadonly={false}
...
test('RelationshipAttribute displays related attribute that allows multiple relationships to be selected.', () => {
  attribute.listMultiSelect = true;
  setup()
  expect(screen.getByText(attribute.description)).toBeTruthy();
});

test('RelationshipAttribute displays related attribute that allows a single relationship to be selected.', () => {
  attribute.listMultiSelect = false;
  setup()
  expect(screen.getByText(attribute.description)).toBeTruthy();
});