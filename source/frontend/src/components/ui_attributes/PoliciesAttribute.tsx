  ...
  function getUpdatedPolicies(event) {
    if (
      ...
      return options
        ...
        .map((valueItem) => {
          return { policy_id: valueItem.value };
        });
    } else if (event.detail.selectedOptions.length > 0) {
      return event.detail.selectedOptions.map((valueItem) => {
        return { policy_id: valueItem.value };
      });
    } else {
      return [];
  ...
  return (
    ...
    >
      <Multiselect
        ...
        disabled={isReadonly}
        onChange={(event) => handleUpdate(attribute, getUpdatedPolicies(event), errorText)}
      ...
      {resolvedOptions ? (
        <div>
          <Spinner size="normal" /> Resolving IDs..{" "}
        </div>
      ) : undefined}
    </FormField>
  );
