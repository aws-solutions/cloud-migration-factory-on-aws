...
type AllViewerAttributesParams = {
  item: any; schemas: Record<string, EntitySchema>;
  dataAll: BaseData;
  schema: EntitySchema;
  hideEmpty?: boolean;
};
const AllViewerAttributes = ({dataAll, hideEmpty, item, schema: {attributes}, schemas}: AllViewerAttributesParams) => {
  ...
  function addListAttribute(attribute: Attribute) {
    const attributeValue = getNestedValuePath(item, attribute.name);
    return (
      attributeValue || displayEmpty
        ?
  ...
  function createMutipleRelationshipList(actualValuesNames: { [x: string]: string; }, actualValue: { [x: string]: string | number | boolean }) {
    let multipleApps = [];
  ...
  function getMultiRelationshipDisplayValues(relatedSchema: string,
                                             attribute: Attribute,
