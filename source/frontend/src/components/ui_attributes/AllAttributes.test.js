...
const setup = () => {
  const comp =  render(
    ...
    />)
  const input = screen.getByLabelText('server_name')
  return {
    ...comp,
    input
  }
...
test('AllAttributes displays a string attribute', () => {
  setup()
  expect(screen.getByText('Server Name')).toBeTruthy();
});

test('AllAttributes enter string to trigger update', () => {
  const {input} = setup()
  fireEvent.change(input, {target: {value: 'value'}})
  expect(item['server_name']).toBe('value')
});