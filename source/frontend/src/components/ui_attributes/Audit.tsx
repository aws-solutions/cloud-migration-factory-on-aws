          ...
          <ColumnLayout columns={2} variant="text-grid">
              <div>
                  <SpaceBetween size="l">
                      <TextAttribute
                          label={'Created by'}
                      >{getHistoryUser(item,'createdBy')}</TextAttribute>
                      <TextAttribute
                          label={'Created on'}
                      >{getHistoryDate(item, 'createdTimestamp')}</TextAttribute>
                  </SpaceBetween>
              </div>
              <div>
                  <SpaceBetween size="l">
                      ...
                      >{getHistoryUser(item,'lastModifiedBy')}</TextAttribute>
                      <TextAttribute
                          label={'Last updated on'}
                      >{getHistoryDate(item, 'lastModifiedTimestamp')}</TextAttribute>
                  </SpaceBetween>
              </div>
          </ColumnLayout>
  )
