...
const SchemaAttributeConditionsEdit = ({
  ...
  handleUserInputEditSchemaConditions,
  schemaAttributes,
  editDisabled,
}) => {
  //Selection options.
  const outcomeValues = [
    ...
    { label: "Not hidden", value: "not_hidden" },
  ];
  ...
  function getPlaceHolder(outcome) {
    if (
      editingSchemaConditionsTemp?.outcomes &&
      editingSchemaConditionsTemp.outcomes[outcome] &&
      editingSchemaConditionsTemp.outcomes[outcome].length > 0
    ) {
  ...
  if (editingSchemaConditionsTemp.queries) {
      ...
      return (
            ...
            <Header
              ...
              actions={
                <SpaceBetween direction="horizontal" size="xs">
                  <Button
                    onClick={() => handleUserInputEditSchemaConditions(queriesKey + "." + index + "", undefined)}
        ...
        >
          <SpaceBetween size={"xl"} direction={"vertical"}>
            <FormField label={messages.attributeFieldNameLabel}>
              <Select
                selectedOption={query.attribute ? { value: query.attribute, label: query.attribute } : ""}
                onChange={(event) =>
                  handleUserInputEditSchemaConditions(
                    queriesKey + "." + index + "." + queriesAttributeKey,
                    event.detail.selectedOption.value
                  )
                }
                options={schemaAttributes.map((item) => {
            ...
            <FormField label={messages.comparatorFieldNameLabel}>
              <Select
                selectedOption={query.comparator ? { value: query.comparator, label: query.comparator } : ""}
                onChange={(event) =>
                  handleUserInputEditSchemaConditions(
                    queriesKey + "." + index + "." + queriesComparatorKey,
                    event.detail.selectedOption.value
                  )
                }
            ...
            {query.comparator !== "!empty" && query.comparator !== "empty" ? (
              <FormField label={messages.valueFieldNameLabel}>
                <Input
                  onChange={(event) =>
                    handleUserInputEditSchemaConditions(
                      queriesKey + "." + index + "." + queriesValueKey,
                      event.detail.value
                    )
                  }
    ...
    outcomes.push(
      <Container
        key={outcomesKey + "." + trueOutcomeKey}
        header={
          <Header variant="h2" actions={<SpaceBetween direction="horizontal" size="xs"></SpaceBetween>}>
            Then...
          </Header>
        }
      >
        <FormField label={messages.thenFieldNameLabel}>
          <Multiselect
            selectedOptions={getSelectedOutcomeOptions(trueOutcomeKey)}
            onChange={(event) =>
              handleUserInputEditSchemaConditions(
                outcomesKey + "." + trueOutcomeKey,
                event.detail.selectedOptions
                  ? event.detail.selectedOptions.map((item) => {
                      return item.value;
                    })
                  : []
              )
    ...
    outcomes.push(
      <Container
        key={outcomesKey + "." + falseOutcomeKey}
        header={
          <Header variant="h2" actions={<SpaceBetween direction="horizontal" size="xs"></SpaceBetween>}>
            Else...
          </Header>
      ...
      >
        <FormField label={messages.elseFieldNameLabel}>
          <Multiselect
            selectedOptions={getSelectedOutcomeOptions(falseOutcomeKey)}
            onChange={(event) =>
              handleUserInputEditSchemaConditions(
                outcomesKey + "." + falseOutcomeKey,
                event.detail.selectedOptions
                  ? event.detail.selectedOptions.map((item) => {
                      return item.value;
                    })
  ...
  if (editingSchemaConditionsTemp) {
    return (
      <Container header={<Header variant="h2">Conditions and Outcomes</Header>}>
        <SpaceBetween direction={"vertical"} size={"xs"}>
          <Container
            header={
              <Header
                ...
                actions={
                  <SpaceBetween direction="horizontal" size="xs">
                    <Button
                      onClick={() => handleUserInputEditSchemaConditions(queriesKey + ".+1", {})}
                      disabled={editDisabled}
                    >
