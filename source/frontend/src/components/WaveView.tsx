...
export const WaveDetailsView = ({dataAll, schemas, selectedItems}: WaveDetailsViewParams) => {
  ...
  const appsForCurrentWave = dataAll.application.data.filter(
    app => app.wave_id === selectedWaveId
  );

  const serversForCurrentWave = appsForCurrentWave.flatMap(
    app => dataAll.server.data.filter(server => server.app_id === app.app_id)
  );

  const jobsForCurrentWave = dataAll.job.data.filter(
    job => job.script.script_arguments?.Waveid === selectedWaveId
  );
