...
export function getSummary(
  ...
  for (const schemaName in schemas) {
    if (schemas[schemaName].schema_type === 'user') {
      ...
      if (distinct[schemaName].length === 0) { //If nothing returned then continue to next schema.
        continue;
      }
      ...
      let keyAttribute = schemaAttributes.find((attr: { attribute: { name: string; }; }) => { //Get schema key attribute.
...
export function getRelationshipValueType(
  importedAttribute: { import_raw_header: string; attribute: { rel_display_attribute: string; name: string; listMultiSelect: any; }; },
  schemaName: string
) {
...
export function buildCommitExceptionNotification(
  ...
  if (exceptionData) {
    ...
    return ({itemType: schema, error: currentItemElement ? currentItemElement + ' - Unknown error occurred' : 'Unknown error occurred', item: currentItem})
  }
}