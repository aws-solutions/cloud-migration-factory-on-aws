...
STATUS_OK_TO_PROCEED = [STATUS_TXT_COMPLETE, STATUS_TXT_SKIPPED]
...
def update_pipeline_status(pipeline_id, pipeline_status, current_task_id=None):
...
def update_task_execution_status(task_execution_id, task_execution_status):
...
def update_task_execution(task_execution_id, update_expr, update_expr_attributes, update_attribute_values):
    ...
    try:
        ...
        error_code = e.response.get("Error", {}).get("Code")
        if error_code == "ConditionalCheckFailedException":
            logger.debug(f'Task execution no longer exist {task_execution_id}')
        else:
            raise e
...
def handle_lambda_based_task_execution(task, task_execution, auth_context):
    ...
    update_task_execution_status(task_execution_id, STATUS_TXT_IN_PROGRESS)
    ...
    try:
        ...
        if response['StatusCode'] != 200:
            update_task_execution_status(task_execution_id, 'Failed')
...
def handle_manual_task_execution(task_execution):
    ...
    update_task_execution_status(task_execution_id, 'Pending Approval')
...
def check_successors_predecessors_complete(all_task_executions, task_execution_id):
    ...
    if len(current_task_execution) > 0:
        for successor_task_id in current_task_execution[0].get('task_successors', []):
            ...
            if successor_task_execution[0].get('task_execution_status', '{not-set}') != STATUS_TXT_NOT_STARTED:
...
def is_pipeline_complete(all_task_executions):
    ...
    for task in all_task_executions:
        if len(task.get('task_successors', [])) == 0 and task.get('task_execution_status', None) in STATUS_OK_TO_PROCEED:
            # last task in branch and branch is complete.
            completed_branch_task_ids.append(task['task_execution_id'])
        elif len(task.get('task_successors', [])) == 0 and task.get('task_execution_status', None) not in STATUS_OK_TO_PROCEED:
...
def get_incomplete_predecessors(predecessors_tasks):
    ...
    for predecessors_task in predecessors_tasks:
        if predecessors_task.get('task_execution_status', '') not in STATUS_OK_TO_PROCEED:
...
def handle_task_executions(pipeline_id, tasks_execution):
    for task_execution in tasks_execution:
        if task_execution:
            ...
            update_pipeline_status(pipeline_id, STATUS_TXT_COMPLETE)
...
def handle_task_execution(pipeline_id, task_execution):
    ...
    update_pipeline_status(pipeline_id, STATUS_TXT_IN_PROGRESS, task_id)
...
def process_pipeline_event(dynamodb_record):
    ...
    elif dynamodb_record['NewImage']['pipeline_status']['S'] == STATUS_TXT_NOT_STARTED and \
...
def is_valid_failed_update(old_task_execution_status, new_task_execution_status):
    return old_task_execution_status != STATUS_TXT_IN_PROGRESS and new_task_execution_status == STATUS_TXT_FAILED


def is_valid_update(old_task_execution_status, new_task_execution_status):
     return (old_task_execution_status in [STATUS_TXT_IN_PROGRESS, STATUS_TXT_PENDING_APPROVAL] and new_task_execution_status == STATUS_TXT_COMPLETE) or (old_task_execution_status == STATUS_TXT_FAILED and new_task_execution_status == STATUS_TXT_SKIPPED)


def is_valid_retry(old_task_execution_status, new_task_execution_status):
    return old_task_execution_status in STATUS_OK_TO_RETRY and new_task_execution_status == STATUS_TXT_RETRY
...
def process_task_execution_event(dynamodb_record):
    ...
    new_task_execution_status = new_dynamodb_record['task_execution_status']['S']
    old_task_execution_status = dynamodb_record['OldImage']['task_execution_status']['S']
    ...
    if is_valid_failed_update(old_task_execution_status, new_task_execution_status):
        ...
        update_pipeline_status(pipeline_id, STATUS_TXT_FAILED)
        ...
        if len(already_processed_successor_tasks) > 0:
            logger.warning(f'Successors will not be processed, as already in state other than {STATUS_TXT_NOT_STARTED}: {",".join(incomplete_predecessor_task_ids)}')
